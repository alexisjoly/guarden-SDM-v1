...
class InceptionEnv(nn.Module):
    ...
    def __init__(self, n_labels=3336, n_input=107, n_features=8, n_env=4, dropout=0.5, last_layer=True, logit=False, sigmoid=False, exp=False,
                 normalize_weight=1., temperature=1., predict_function=None, kernel_size=7):
        ...
        self.reduction = nn.Linear(2048, n_features)
        self.fc = nn.Linear(n_features + n_env, n_labels)
    ...
    def forward(self, x):
        ...
        # 2048 x 31 x 31
        x = F.avg_pool2d(x, kernel_size=self.kernel_size)
        ...
        if self.last_layer:
            # 2048
            x = self.fc(torch.cat((x, x_env), 1))
            # (num_classes)
            if not self.training and not self.logit and not self.exp and not self.sigmoid and not self.predict_function:
                x = F.softmax(x*self.temperature, dim=-1)
            elif not self.training and not self.logit and self.exp:
                x = x.exp()
            elif not self.training and not self.logit and self.sigmoid:
                x = F.sigmoid(x)
            elif not self.training and not self.logit and self.predict_function:
                x = self.predict_function(x)
